...
/** Retab API keys start with this prefix; simple check to prevent unauthorized/wrong keys */
const RETAB_API_KEY_PREFIX = "sk_retab";
...
/**
 * Workflow step card for the quick start guide (with staggered animation)
 */
function WorkflowStep({ icon: Icon, label, description, accent, delay = 0 }) {
  return (
    <div
      className="flex flex-col items-center text-center gap-2 flex-1 min-w-0 anim-fade-in-up"
...
/**
 * Staggered child: fades + slides up with a delay
 */
...
export default function OnboardingModal({
  ...
  const [sailboatKey, setSailboatKey] = useState(0); // increment to spawn a new sailboat

  const canAdvance = () => {
    if (step === 1) return usernameInput.trim().length > 0;
    if (step === 2) {
      const key = apiKeyInput.trim();
      return key.length > 0 && key.startsWith(RETAB_API_KEY_PREFIX);
    }
    return true;
  };
  ...
  // Animation class for step content — direction-aware slide + fade.
  // key={step} on the wrapper ensures React re-mounts so animation replays.
  const stepAnim = step === 0
    ? "anim-fade-in-scale"
    : direction === "back"
      ? "anim-slide-left"
      : "anim-slide-right";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 anim-fade-in">
      {/* Backdrop */}
      <div className="absolute inset-0 bg-black/60 dark:bg-black/70 anim-fade-in" />

      {/* Sailboat easter egg — sails across the screen when SAIL is clicked */}
      {sailboatKey > 0 && (
        <div
          key={sailboatKey}
          className="fixed top-1/2 left-0 z-60 anim-sailboat select-none"
          aria-hidden="true"
        >
          <span className="anim-sailboat-inner inline-block text-4xl" role="img" aria-label="sailboat">&#9973;</span>
        </div>
      )}

      {/* Dialog */}
      <div className="relative bg-white dark:bg-neutral-900 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden anim-fade-in-scale">
        {/* Stewart Red accent bar */}
        <div className="h-1 bg-linear-to-r from-[#9e2339] via-[#c4354d] to-[#9e2339]" />

        {/* Content — key={step} forces full re-mount so CSS animation replays */}
              ...
              <div className="text-center">
                ...
                <Stagger delay={380}>
                  ...
                  <div className="flex items-center justify-center gap-1.5 mb-8">
                    ...
                    <button
                      type="button"
                      onClick={() => setSailboatKey((k) => k + 1)}
                      className="text-[11px] tracking-wide font-bold text-[#9e2339] hover:text-[#c4354d] transition-colors cursor-pointer bg-transparent border-none p-0 m-0"
                    >
                      SAIL
                    </button>
                  </div>
